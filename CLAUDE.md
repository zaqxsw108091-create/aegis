...
- [x] P5 대시보드: 관리자(ROLE_ADMIN) 전용 모니터링 REST API(이벤트/차단IP/통계/메트릭/사용자별 실패횟수) + 관제 콘솔 화면
      (외부 CSS로 CSP 유지, 위협 배너, 30초 갱신) + 관리자 조치(수동 IP 차단/해제, 계정 잠금 해제; /admin/** CSRF 보호, 감사 기록)
...
- [x] P9 보안 알림: IP 차단/토큰 재사용 시 웹훅 통지(AEGIS_ALERT_WEBHOOK, 기본 비활성, 비동기·실패 안전)
      · 후속(선택): Spotless/Checkstyle, 분산 레이트리밋(Redis 구현체), 2FA(TOTP), 대시보드 시간대별 그래프
