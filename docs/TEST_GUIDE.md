...
### 보호 자원 호출 2가지 방법
- **Basic 인증(가장 쉬움)**: `curl.exe -u admin:admin1234 ...`
...
```powershell
curl.exe -s -o NUL -w "%{http_code}`n" -u user:user1234 http://localhost:8080/api/admin/dashboard/stats
```
...
```powershell
curl.exe -s -u admin:admin1234 http://localhost:8080/api/admin/dashboard/stats
curl.exe -s -u admin:admin1234 http://localhost:8080/api/admin/dashboard/events
curl.exe -s -u admin:admin1234 http://localhost:8080/api/admin/dashboard/blocked-ips
```
...
- Basic 인증 창이 뜨면 `admin` / `admin1234` 입력 → 대시보드 화면이 뜬다.
...
```powershell
curl.exe -s -u admin:admin1234 http://localhost:8080/actuator/prometheus | findstr aegis_detection
```
...
- **검증 가능한 보안**: 무차별 대입/레이트리밋/권한거부/보안헤더를 **자동화 테스트 34개**로 보증.
